...
class radexgrid:
    ...
    def interpolate_indices(self, j, width, temp, rho, sigma):
        """Returns the values required for the interpolation."""
        w = np.interp(width, self.linewidth, np.arange(self.vals[2]))
        t = np.interp(temp, self.temperature, np.arange(self.vals[5]))
        r = np.interp(rho, self.logdensity, np.arange(self.vals[8]))
        s = np.interp(sigma, self.logsigma, np.arange(self.vals[11]))
        return [[w], [t], [r], [s]]

    def interpolate_intensity(self, j, width, temp, rho, sigma):
        """Interpolate the intensity grid."""
        indices = self.interpolate_indices(j, width, temp, rho, sigma)
        return map_coordinates(self.grid[j,0], indices, order=1, mode='nearest')

    def interpolate_tau(self, j, width, temp, rho, sigma):
    ...
    def get_spectra(self, j, temp, sigma, rho, centre, width, velax):
        """Single Gaussian spectrum."""
        Tb = self.interpolate_intensity(j, width * self.fwhm, temp, rho, sigma)
        return self.gaussian(velax, Tb, centre, width)
